...
try:
    ...
    pd = None
    px = None
    st.warning("⚠️ As bibliotecas 'pandas' e 'plotly' não foram encontradas. A aba de gráficos pode não funcionar. Verifique seu 'requirements.txt'.")
...
def registrar_tramitacao(processo_id, setor, data_entrada, data_saida=None, observacao=""):
    ...
    Atualiza a data_saida da tramitação anterior se houver uma aberta."""
    if not conn: return False, "❌ Erro de conexão com o banco!"
    try:
        c = conn.cursor()
        # Fecha a tramitação anterior (se houver uma aberta)
...
def atualizar_status(processo_id, novo_status):
    ...
    try:
        ...
        # Corrigido: Removido parêntese extra que causava SyntaxError
        return False, f"❌ Erro ao atualizar status: {str(e)}" 

def get_processos_df():
    """Carrega todos os processos para um DataFrame do pandas."""
    if not conn or pd is None: return pd.DataFrame() # Verifica se pandas foi importado
    try:
...
with st.sidebar:
    ...
    if api_key_input:
        ...
        st.session_state['api_key'] = '' # Garante que seja uma string vazia se nada for inserido
...
with tab1:
    ...
    with st.form("cadastro_processo"):
        ...
        with col1:
            # Removido required=True para compatibilidade com versões mais antigas do Streamlit
            ...
            analista = st.text_input("Nome do Analista Responsável")
        with col2:
            uso = st.selectbox("Uso", usos_options)
            tipologia = st.selectbox("Tipologia", tipologias_options)
            area = st.number_input("Área Construída (m²)", min_value=0.0, value=0.0, step=0.01)
        ...
        for p in processos:
            ...
            with st.expander(f"Processo: {p[1]} - Requerente: {p[3]} - Status: {p[9]}"):
                ...
                if not st.session_state.get(f"edit_mode_{p[0]}", False):
                    ...
                    with col_edit:
                        if st.button("✏️ Editar Processo", key=f"edit_btn_{p[0]}", use_container_width=True):
                            st.session_state[f"edit_mode_{p[0]}"] = True
                            st.experimental_rerun()

                    with col_del:
                        if st.button("🗑️ Deletar Processo", key=f"delete_btn_{p[0]}", type="secondary", use_container_width=True):
                            # Confirmação de deleção
                            st.warning(f"Tem certeza que deseja deletar o processo {p[1]}? Esta ação é irreversível e também apagará todas as tramitações e análises associadas.")
                            if st.button("CONFIRMAR DELEÇÃO", key=f"confirm_delete_btn_{p[0]}", type="danger"):
                ...
                # Se estiver em modo de edição, mostra o formulário de edição
                    ...
                    with st.form(f"form_editar_processo_{p[0]}"):
                        ...
                        if col_save.form_submit_button("Salvar Alterações", type="primary"):
                            # Validação manual dos campos editados
                            if not (ed_numero and ed_rt and ed_requerente and ed_analista and ed_uso and ed_tipologia and ed_area > 0 and ed_data_protocolo):
                                st.error("❌ Por favor, preencha todos os campos obrigatórios e garanta que a área seja maior que zero.")
                            else:
...
with tab3:
    ...
    if not processos_tramitacao:
        ...
        # Cria uma lista de tuplas (id, numero) para o selectbox
        processo_options = [(p[0], p[1]) for p in processos_tramitacao]

        # Garante que o valor padrão do selectbox seja o primeiro processo, se houver
        default_index = 0
        ...
        processo_selecionado_tuple = st.selectbox(
            ...
            format_func=lambda x: f"{x[1]}", # Exibe apenas o número do processo
            index=default_index,
        ...
        if processo_selecionado_tuple:
            ...
            st.session_state['selected_processo_tramitacao_id'] = pid_tramitacao # Salva o ID selecionado

            processo_dados = buscar_por_numero(processo_selecionado_tuple[1]) # Busca todos os dados do processo
