...
def main():
    ...
    if not st.session_state['logged_in']:
        ...
        with st.form("login"):
            ...
            if st.form_submit_button("Entrar"):
                ...
                # Aceita admin ou seu usuário específico
    ...
    # [CORREÇÃO 1] Adicionado o fechamento do colchete que faltava
        ...
        if procs:
            ...
            with st.form("new_tram"):
                ...
                if st.form_submit_button("Movimentar", type="primary"):
                    # [CORREÇÃO 2] Lógica corrigida para garantir fechamento e abertura de etapas
                    if not tem_saida:
            ...
            if suc:
                ...
                if rows:
                    ...
                    # [CORREÇÃO 3] Ordem Analógica (do antigo para o novo: ascending=True)
                        ...
                        if r:
                            with st.form(f"edit_tram_{tid}"):
                                ...
                                with ec4:
                                    ...
                                    edtsai = st.date_input("Data Saída", datetime.strptime(r[4], '%Y-%m-%d').date()) if has_exit and r[4] else None
            ...
            if st.button("Analisar") and up_p and up_l:
                with st.spinner("Processando..."):
                    try:
                        ...
                        # [CORREÇÃO 4] Nomes corretos e prioridade para assinatura paga
                        # SEM o prefixo "models/" que estava dando erro 404
                        ...
                        for m_nome in modelos:
                            try:
                                ...
                                LEI: {txt_l[:50000]}
                                PROJETO: {txt_p[:50000]}
                                Responda com: 1. Resumo, 2. Conformidade, 3. Pendências, 4. Conclusão.
                                """)
                                break
                            except Exception:
                                time.sleep(1) # Pausa rápida se der erro de cota
                                continue
                        
                        if resultado:
                            st.markdown(resultado.text)
                        else:
                            st.error("Erro de conexão. Verifique se sua API Key tem permissão para o modelo Gemini 1.5 Pro.")
        ...
        if pd is not None and px is not None:
            ...
            if not df.empty:
                ...
                with row2_2:
                    ...
                    try:
                        ...
                        if not df_tram_all.empty:
                            ...
                            st.plotly_chart(px.pie(df_tram_all, values='dias', names='setor', title='Tempo Total'), use_container_width=True)
                    except: pass
                ...
                if not df_analista.empty:
                    st.plotly_chart(px.bar(df_analista, x='area', y='analista', orientation='h', text_auto='.0f'), use_container_width=True)
