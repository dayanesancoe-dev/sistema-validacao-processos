...
def main():
    ...
    if conn and pd is not None:
        ...
        if os.path.exists("processos.db"):
            with open("processos.db", "rb") as f:
                st.sidebar.download_button(label="📦 Baixar Backup (.db)", data=f, file_name=f"backup_{datetime.now().strftime('%Y%m%d_%H%M')}.db", mime="application/octet-stream")
    ...
    # === TIPOLOGIAS ATUALIZADAS CONFORME SOLICITADO ===
    ...
    with tab1:
        ...
        with st.form("novo_proc"):
            ...
            if st.form_submit_button("Salvar Processo"):
                suc, msg = executar_query('INSERT INTO processos (numero, rt, requerente, analista, uso, tipologia, area, data_protocolo) VALUES (?,?,?,?,?,?,?,?)',
    ...
    # --- ABA 3: TRAMITAÇÃO (ORDEM CRONOLÓGICA) ---
    with tab3:
    ...
    with tab6:
        ...
        if pd is not None and px is not None:
            df = get_processos_df()
            if not df.empty:
                st.plotly_chart(px.pie(df, names='status', title='Status dos Processos'), use_container_width=True)
